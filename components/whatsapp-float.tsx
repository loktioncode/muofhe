...
export function WhatsAppFloat() {
  ...
  return (
    <a
      ...
      aria-label={`Chat on WhatsApp with ${SITE_NAME}`}
      className={[
        "fixed z-[90] flex h-14 w-14 items-center justify-center rounded-full shadow-lg",
        "bottom-[max(1.25rem,env(safe-area-inset-bottom,0px))] right-[max(1.25rem,env(safe-area-inset-right,0px))]",
        "bg-[#25D366] text-white ring-2 ring-white/90",
        "transition-transform duration-200 hover:scale-110 hover:bg-[#20BD5A] active:scale-95",
